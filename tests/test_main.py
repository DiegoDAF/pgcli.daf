...
def test_execute_statements_watch_repeats_only_its_own_statement(executor):
    ...
    outputs = [c[0][0] for c in mock_echo.call_args_list]
    # select 111 once, then select 222 repeated by \watch until the Ctrl-C.
...
def test_execute_statements_on_error_resume_continues(executor):
    ...
    cli = PGCli(pgexecute=executor, on_error="RESUME")
    with mock.patch.object(cli, "echo_via_pager") as mock_echo:
        ok = cli._execute_statements("select boom_not_a_column;\nselect 444;")
    assert ok is False
    outputs = [c[0][0] for c in mock_echo.call_args_list]
...
def test_file_mode_failed_statement_exits_nonzero(tmpdir):
    """-f with a failed statement and on_error = STOP exits 1, like psql
    with ON_ERROR_STOP."""
    sql_file = tmpdir.join("script.sql")
    sql_file.write("select boom;")
    cli = PGCli(pgclirc_file=str(tmpdir.join("rcfile")))
    cli.input_files = [str(sql_file)]
    with mock.patch.object(cli, "_execute_statements", return_value=False):
        with pytest.raises(SystemExit) as e:
            cli.run_cli()
    assert e.value.code == 1


def test_command_mode_failed_statement_exits_nonzero(tmpdir):
    """-c behaves the same as -f on failure."""
    cli = PGCli(pgclirc_file=str(tmpdir.join("rcfile")))
    cli.commands = ["select boom;"]
    with mock.patch.object(cli, "_execute_statements", return_value=False):
        with pytest.raises(SystemExit) as e:
            cli.run_cli()
    assert e.value.code == 1
